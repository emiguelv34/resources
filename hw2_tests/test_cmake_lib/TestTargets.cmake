...
set(MAKE_GRADE_COMMAND_BODY "" CACHE INTERNAL "Commands for grade target" FORCE)
...
function(add_executable_problem)
	...
	# build executable
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${ALS_NAME}, output saved to ${COMPILE_LOG_DIR}/${ALS_NAME}.complog" CACHE INTERNAL "Commands for grade target" FORCE)
    set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make hw${HW_NUMBER}_${ALS_NAME} 1>${COMPILE_LOG_DIR}/${ALS_NAME}.complog 2>&1 || true CACHE INTERNAL "Commands for grade target" FORCE)

    # build tests
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${ALS_NAME} tests" CACHE INTERNAL "Commands for grade target" FORCE)
    set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make ${ALS_NAME}_tests || true CACHE INTERNAL "Commands for grade target" FORCE)
...
function(add_source_problem)
	...
	# build user code
	set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${ASP_NAME}, output saved to ${COMPILE_LOG_DIR}/${ASP_NAME}.complog" CACHE INTERNAL "Commands for grade target" FORCE)
    set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND make hw${HW_NUMBER}_${ASP_NAME} 1>${COMPILE_LOG_DIR}/${ASP_NAME}.complog 2>&1 || true CACHE INTERNAL "Commands for grade target" FORCE)

    # build tests
	set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${ASP_NAME} tests" CACHE INTERNAL "Commands for grade target" FORCE)
    set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND make ${ASP_NAME}_tests || true CACHE INTERNAL "Commands for grade target" FORCE)

	if(ASP_RUNTIME_TEST_SOURCE)
		# build runtime tests
		set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${ASP_NAME} runtime tests" CACHE INTERNAL "Commands for grade target" FORCE)
		set(MAKE_GRADE_COMMAND_BODY "${MAKE_GRADE_COMMAND_BODY}" COMMAND make ${ASP_NAME}_runtime_tests || true CACHE INTERNAL "Commands for grade target" FORCE)
	endif()
...
function(add_makefile_problem)
	...
	add_custom_command(OUTPUT ${USER_EXECUTABLE}
		COMMAND make ${AMP_MAKE_TARGET} CXXFLAGS=-g\ -Wall\ -std=c++14
	...
	# build executable
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${AMP_NAME} using Makefile, output saved to ${COMPILE_LOG_DIR}/${AMP_NAME}.complog" CACHE INTERNAL "Commands for grade target" FORCE)
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make run_make_${AMP_NAME} 1>${COMPILE_LOG_DIR}/${AMP_NAME}.complog 2>&1 || true CACHE INTERNAL "Commands for grade target" FORCE)

	# build tests
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${AMP_NAME} tests" CACHE INTERNAL "Commands for grade target" FORCE)
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make ${AMP_NAME}_tests || true CACHE INTERNAL "Commands for grade target" FORCE)
...
function(add_header_problem)
	...
	# build tests
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${AHP_NAME} tests, output saved to ${COMPILE_LOG_DIR}/${AHP_NAME}.complog" CACHE INTERNAL "Commands for grade target" FORCE)
	set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make ${AHP_NAME}_tests 1>${COMPILE_LOG_DIR}/${AHP_NAME}.complog 2>&1 || true CACHE INTERNAL "Commands for grade target" FORCE)

	if(NOT "${AHP_RUNTIME_TEST_SOURCE}" STREQUAL "")
		# build runtime tests
		set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND ${CMAKE_COMMAND} -E echo "-- Compiling ${AHP_NAME} runtime tests" CACHE INTERNAL "Commands for grade target" FORCE)
		set(MAKE_GRADE_COMMAND_BODY ${MAKE_GRADE_COMMAND_BODY} COMMAND make ${AHP_NAME}_runtime_tests || true CACHE INTERNAL "Commands for grade target" FORCE)
	endif()
